# Telegram RAG Bot — ответы на новости из Telegram с векторным поиском

RAG-бот для Telegram, который отвечает на вопросы на основе реальных данных из каналов с указанием источников.  
Агрегирует и анализирует новостные и криптоданные в реальном времени.

**Стек:** Node.js, n8n, Supabase (pgvector), OpenAI API, Telegram API

---

## 🎯 Зачем этот проект

Telegram-каналы содержат большое количество неструктурированных новостей.  
Этот проект решает:

- Перегрузку информацией  
- Отсутствие поиска по каналам  
- Отсутствие ответов с источниками  

Бот агрегирует, фильтрует и суммаризирует новости с помощью RAG.


---

## 🧠 Дополнительный сценарий — Daily Digest

Помимо ответов на вопросы в реальном времени, система может автоматически формировать ежедневные дайджесты на основе заранее заданных аналитических вопросов.

Вместо ручных запросов пользователь задаёт структуру анализа заранее.  
Вопросы хранятся в Google Таблице и обрабатываются по расписанию.

### 📄 Примеры вопросов

Ниже приведены примеры вопросов для автоматического анализа:

- последние новости по XRP, ETH  
- обзор крипторынка с учётом макрофакторов  
- ситуация в экономике США и на рынке труда  
- анализ секторов на Кипре (forex, crypto, igaming)

Полный список доступен в файле:
...
1. В Google Sheets хранится список вопросов (столбец `questions`)
2. n8n запускается по расписанию (например, в 07:15)
3. Каждый вопрос обрабатывается отдельно
4. Для каждого вопроса:
   - создаётся embedding  
   - выполняется векторный поиск  
   - GPT формирует структурированный ответ  
5. Ответы отправляются в Telegram канал
...
```
последние новости по токену XRP
```
...
#### Рост (+)
1. Настроения по XRP на максимуме за 2 года (1)  
2. Приток капитала на фоне оттока из BTC и ETH (1)  

#### Падение (-)
1. Сильное сопротивление на уровне $1.40 (1)  
2. Скептицизм по долгосрочной цене (4)  

### итог
Итоговая оценка: +6  
Позитивные сигналы роста присутствуют, но есть ограничения. Рекомендуется держать.

📡 Sources:
(1) CryptoFateev — 2 мая  
(2) Neo Life XRP — 29 апр.  
(3) Ripple XRP Channel — 29 апр.  
(4) Hobby Crypto X — 2 мая  
```
...
1. Создайте новую Google Таблицу со столбцом:
   ```
   questions
   ```

2. Добавьте вопросы (по одному на строку)
...
**Вопрос**
> Что произошло с биткоином сегодня?

**Ответ**

1. Биткоин закрылся выше $75,000 впервые за 73 дня. (1)  
2. Майкл Сэйлор купил биткоин почти на $1 млрд. (1)(2)  
3. ETF на биткоин показали максимальный приток с января. (2)  
...
- RAG-поиск по Telegram-данным  
- Ответы с указанием источников  
- Дедупликация перед эмбеддингами  
- Векторный поиск (pgvector)  
- Конфиг через Google Sheets  
- История диалога  
...
Telegram-каналы → Node.js сервер → n8n → Supabase → GPT → Telegram бот
